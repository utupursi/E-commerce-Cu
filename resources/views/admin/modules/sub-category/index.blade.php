...
@section('content')
    {!! Form::open(['url' => route('subCategoryIndex',app()->getLocale()),'method' =>'get']) !!}
    <div class="controls-above-table">
        <div class="row">
            <div class="col-sm-2">
                <a class="btn btn-lg btn-success"
                   href="{{route('subCategoryCreateView',app()->getLocale())}}">@lang('admin.create_sub_category')</a>
            </div>
                ...
                @foreach($categories as $category)
                    <tr>
                        ...
                        <td class="text-center">
                            @if($category->status)
                                <span class="text-green">@lang('category.on')</span>
                            @else
                                <span class="text-red">@lang('category.off')</span>
                            @endif
                        </td>
                        <td class="row-actions d-flex">
                            <a href="{{route('subCategoryShow',[app()->getLocale(),$category->id])}}">
                                <i class="os-icon os-icon-documents-07"></i>
                            </a>
                            <a href="{{route('subCategoryEditView',[app()->getLocale(), $category->id])}}">
                                <i class="os-icon os-icon-ui-49">

                                </i>
                            </a>
                            {!! Form::open(['url' => route('subCategoryDestroy',[app()->getLocale(),$category->id]),'method' =>'delete']) !!}
